...
proc error_interpUi {msg options} {
	...
	if {[winfo exists .error_w] == 0} {
		...
		ttk::button $bf.b_ok -text [mc "OK"] -command "::tk::RestoreFocusGrab $w $w destroy; vid_wmCursor 1"
		...
		$mf.t_info.mContext add command -label [mc "Select everything"] -compound left -image $::icon_men(placeholder) -command [list $mf.t_info tag add sel 0.0 end] -accelerator Ctrl+A
		$mf.t_info.mContext add separator
		$mf.t_info.mContext add command -label [mc "Copy to clipboard"] -compound left -image $::icon_men(clipboard) -command [list event generate $mf.t_info <<Copy>>] -accelerator Ctrl+C
