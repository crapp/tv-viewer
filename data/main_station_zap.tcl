...
proc main_stationListboxStations {slist} {
	...
	if {"[$slist cget -state]" != "disabled"} {
		...
		main_stationInputStart $slist last
	}
}
...
proc main_stationInputStart {slist lasts} {
	...
	if {$status_get_input == 0} {
		if {$::kanalinput([lindex $::station($lasts) 2]) == [lindex $resultat_get_input 3]} {
			...
			set ::main(change_inputLoop_id) [after 200 [list main_stationInputLoop 0 $::kanalinput([lindex $::station($lasts) 2]) [lindex $::station($lasts) 1] [lindex $::station($lasts) 2] $restart 1]]
		}
		if {"$slist" == ".tv.slist.lb_station"} {
			if {[winfo exists .frame_slistbox] == 1} {
				.frame_slistbox.listbox_slist see [expr [lindex $::station($lasts) 2] - 1]
				if {[string trim [.frame_slistbox.listbox_slist curselection]] != {}} {
					.frame_slistbox.listbox_slist selection clear [.frame_slistbox.listbox_slist curselection]
					.frame_slistbox.listbox_slist activate [expr [lindex $::station($lasts) 2] - 1]
				}
				.frame_slistbox.listbox_slist selection set [expr [lindex $::station($lasts) 2] - 1]
			}
		}
